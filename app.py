...
# Function to extract water-related bands
def extract_water_bands(image_path):
    """Extract bands relevant for water detection"""
    try:
        with rasterio.open(image_path) as src:
            # Read specific bands for water detection
            # B2 (Blue), B3 (Green), B4 (Red), B8 (NIR), B11 (SWIR1), B12 (SWIR2)
            band_indices = [2, 3, 4, 8, 11, 12]  # 1-indexed to 0-indexed
            
            water_bands = []
            for band_idx in band_indices:
                if band_idx <= src.count:
                    water_bands.append(src.read(band_idx))
                else:
                    st.warning(f"Band {band_idx} not found in image")
            
            water_bands_img = np.stack(water_bands, axis=0)
            meta = src.meta.copy()
            
        return water_bands_img, meta
        
    except Exception as e:
        st.error(f"Error extracting water bands: {str(e)}")
        return None, None
